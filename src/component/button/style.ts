...
export const getStyle = ({ revers, rotate, className }: IStyleProps) => ({
  btn: cn(
    'pl-3 py-2 pr-6 bg-blueBg border-blueBorder text-white sm:text-darkText border-[1px] w-full rounded-xs flex justify-between ease-in-out duration-300 group/item sm:bg-transparent sm:hover:bg-blueBg cursor-pointer',
  ...
  textStyle: cn(
    'block text-[18px] -translate-y-[2px] sm:group-hover/item:!text-white sm:group-hover/item:-translate-y-[30px] ease-in-out duration-300 ui-btn-text'
  ),
});
